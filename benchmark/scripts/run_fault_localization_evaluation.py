"""Run frozen FL-v1 on the independent Codeflaws evaluation set."""
...
from fault_localization.models import CoverageMatrix, LocalizationInput  # noqa: E402
from fault_localization.pipeline import localize  # noqa: E402


EVALUATED_METHODS = ("ochiai", "ochiai_branch_tiebreak")
...
def _fl_v1_ranking(matrix: CoverageMatrix, buggy_source: str) -> dict[str, object]:
    result = localize(matrix, buggy_source)
    result["method_version"] = "fl-v1"
    result["rankings"] = {
        name: result["rankings"].get(name, []) for name in EVALUATED_METHODS
    }
    return result
...
def main() -> int:
    ...
    if args.case_id:
        ...
        if missing:
            print(f"unknown Evaluation case ids: {sorted(missing)}", file=sys.stderr)
            return 1
    ...
    for index, case in enumerate(cases, start=1):
        ...
        try:
            ...
            ranking = _fl_v1_ranking(matrix, case.get_buggy_source())
            ...
            print(
                ...
                f"{len(matrix.executable_branches)} branch outcomes",
                flush=True,
            )
        ...
        _write_jsonl(
            FAULT_LOCALIZATION_INDEPENDENT_GROUND_TRUTH, ground_truth_records
        )

    print(json.dumps({"cases": len(cases), "coverage_errors": errors}, indent=2))
